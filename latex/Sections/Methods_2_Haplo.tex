...
\enumdesc{Autosomal Inheritance Modelling}{For maternal alleles \textit{m1 m2}, paternal alleles \textit{p1 p2}, and child alleles \textit{c1 c2}; check for genotype equality such that \textit{\{c1,c2\}} exists within the four valid genotype designations \textit{\{ \{m1,p1\}, \{m1,p2\}, \{m2, p1\}, \{m2,p2\} \}}. If a match is found (or multiple) then the parental allele colour groups are pushed onto the child allele colour groups in a mutually-exclusive fashion such that \textit{c1 c2} would only share the same colour groups by chance.}
\end{enumerate}
}
...
As shown in the Autosomal Inheritance implementation on page 43, parental alleles are represented as {p1 p2},{m1 m2} pointers which each represent a possible array or founder allele “colour” groups. The valid child allele {c1 c2} configurations are shown in Table 3 below.
